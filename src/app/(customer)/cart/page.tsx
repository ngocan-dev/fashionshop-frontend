      ...
      <Card>
        ...
        <CardContent className="space-y-4">
              ...
              return (
                <div key={item.id} className="flex flex-col gap-4 rounded-2xl border border-border p-4 md:flex-row md:items-center md:justify-between">
                  ...
                  <div className="flex items-center gap-2">
                        ...
                        quantityMutation.mutate(
                          { itemId: item.id, quantity: Math.max(1, item.quantity - 1) },
                          {
                            onError: (error) => {
                              ...
                              toast.error(apiError.message || 'Unable to update quantity');
                            },
                          },
                        )
                      }
                    ...
                    <Button variant="ghost" size="sm" aria-label="Delete cart item" disabled={isMutating} onClick={() => setConfirmItemId(item.id)}>
          ...
          <div className="flex justify-end">
            <Button asChild disabled={isMutating || cartQuery.data.items.length === 0}><Link href="/checkout">Proceed to checkout</Link></Button>
          </div>
        </CardContent>
      </Card>
      <ConfirmDialog
        ...
        onConfirm={() => {
          ...
          deleteMutation.mutate(confirmItemId, {
            onSuccess: () => toast.success('Item removed'),
            onError: (error) => {
              const apiError = error as ParsedApiError;
              toast.error(apiError.message || 'Unable to remove item');
            },
          });
