...
export function PublicProductsPage({ listingHref = '/shop', debugName = 'public-products-page' }: PublicProductsPageProps) {
  ...
  const [showSlowNetworkError, setShowSlowNetworkError] = useState(false);
  ...
  useEffect(() => {
    ...
    console.debug('[PublicProductsPage] query state', {
      debugName,
      pathname,
      ...
      dataLength: productsQuery.data?.length ?? 0,
      params: {
        keyword: searchTerm.trim() || undefined,
        categoryId: selectedCategory !== 'All Products' ? selectedCategory : undefined,
        sortBy: sortValueMap[sortBy],
  ...
  useEffect(() => {
    if (!productsQuery.isPending) {
      setShowSlowNetworkError(false);
      return;
    }

    const timer = window.setTimeout(() => {
      setShowSlowNetworkError(true);
    }, 12000);
  ...
  if (productsQuery.isError || showSlowNetworkError) {
    return (
          ...
          <EmptyState
            ...
            description={
              showSlowNetworkError
                ? 'The products API is taking too long to respond. Please check backend availability and try again.'
                : 'Please try again in a moment.'
            }
            actionLabel="Retry"
            actionHref={listingHref}
          />
        </div>
      </main>
    );
